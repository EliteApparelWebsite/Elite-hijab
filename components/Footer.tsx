...
export default function Footer() {
  ...
  return (
    <footer className="relative bg-[#0A0A0A] border-t border-[#D4AF37]/25">
      {/* Newsletter Section */}
      <div className="border-b border-[#D4AF37]/20 bg-[#0A0A0A]">
        <div className="max-w-wrap mx-auto px-5 md:px-8 py-12 md:py-16">
          <div className="flex flex-col md:flex-row items-center justify-between gap-8 bg-white/[0.04] rounded-3xl p-8 md:p-10 shadow-2xl border border-[#D4AF37]/30 backdrop-blur-md">
            <div className="max-w-md text-center md:text-left">
              <h3 className="font-display font-semibold text-2xl text-white mb-2">Join the Elite Hijab Family</h3>
            ...
            <form className="w-full md:w-auto flex-1 max-w-md flex flex-col gap-2" onSubmit={handleSubmit}>
              <div className="w-full flex gap-3 flex-wrap">
                <input 
                  ...
                  className="flex-1 bg-white/5 border border-[#D4AF37]/40 rounded-full px-6 py-3.5 text-sm text-white placeholder:text-white/40 focus:outline-none focus:border-[#D4AF37] focus:bg-white/10 transition-all disabled:opacity-50"
                  required
                />
                <button 
                  type="submit"
                  disabled={isSubmitting}
                  className="bg-gradient-to-r from-[#DFBA73] via-[#D4AF37] to-[#AA8034] hover:brightness-110 flex-1 lg:flex-none text-black px-8 py-3.5 rounded-full text-sm font-bold transition-all duration-300 shadow-md hover:shadow-lg disabled:opacity-75 disabled:cursor-not-allowed flex items-center justify-center min-w-[120px]"
                >
                  {isSubmitting ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-10 lg:gap-8">
          ...
          <div className="lg:col-span-4 pr-0 lg:pr-8">
            ...
            <div className="flex items-center gap-3">
              <a href={`https://instagram.com/${SITE.instagram}`} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full bg-white/10 border border-white/20 flex items-center justify-center text-white/80 hover:bg-gold hover:text-black hover:border-gold transition-all shadow-sm hover:-translate-y-1 backdrop-blur-sm">
              ...
              <a href={`https://wa.me/${SITE.whatsapp}`} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full bg-white/10 border border-white/20 flex items-center justify-center text-white/80 hover:bg-[#25D366] hover:text-white hover:border-[#25D366] transition-all shadow-sm hover:-translate-y-1 backdrop-blur-sm">
            ...
            <ul className="space-y-3.5">
              {navLinks.map((link) => (
                <li key={link.label}>
                  <Link
                    href={link.href}
                    className="text-white/75 text-sm hover:text-gold transition-colors flex items-center gap-2 group"
                  >
          ...
          <div className="lg:col-span-2">
            ...
            <ul className="space-y-3.5">
              <li>
                <Link href="/policies/privacy" className="text-white/75 text-sm hover:text-gold transition-colors flex items-center gap-2 group">
                  <span className="w-1.5 h-1.5 rounded-full bg-gold/40 group-hover:bg-gold transition-colors"></span>
                  Privacy Policy
                </Link>
              </li>
              <li>
                <Link href="/policies/terms" className="text-white/75 text-sm hover:text-gold transition-colors flex items-center gap-2 group">
                  <span className="w-1.5 h-1.5 rounded-full bg-gold/40 group-hover:bg-gold transition-colors"></span>
                  Terms & Conditions
                </Link>
              </li>
              <li>
                <Link href="/policies/return-and-refund" className="text-white/75 text-sm hover:text-gold transition-colors flex items-center gap-2 group">
                  <span className="w-1.5 h-1.5 rounded-full bg-gold/40 group-hover:bg-gold transition-colors"></span>
                  Return & Refund Policy
                </Link>
              </li>
              <li>
                <Link href="/policies/shipping" className="text-white/75 text-sm hover:text-gold transition-colors flex items-center gap-2 group">
                  <span className="w-1.5 h-1.5 rounded-full bg-gold/40 group-hover:bg-gold transition-colors"></span>
                  Shipping Policy
                </Link>
              </li>
              <li>
                <Link href="/orders/track" className="text-white/75 text-sm hover:text-gold transition-colors flex items-center gap-2 group">
                  <span className="w-1.5 h-1.5 rounded-full bg-gold/40 group-hover:bg-gold transition-colors"></span>
                  Track Order &amp; History
                </Link>
              </li>
              <li>
                <Link href="/faq" className="text-white/75 text-sm hover:text-gold transition-colors flex items-center gap-2 group">
                  <span className="w-1.5 h-1.5 rounded-full bg-gold/40 group-hover:bg-gold transition-colors"></span>
                  FAQ
                </Link>
              </li>
