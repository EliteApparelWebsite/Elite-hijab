      ...
      <div className="max-w-wrap mx-auto px-5 sm:px-8 relative z-10 overflow-visible">
        ...
        <Reveal className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6 pb-8 border-b border-gold/20 overflow-visible">
          <div className="text-center lg:text-left max-w-2xl overflow-visible">
            ...
            {/* Pill Eyebrow */}
            <div className="inline-flex items-center gap-2.5 px-4 py-1.5 rounded-full bg-white/90 backdrop-blur-md border border-[#D4AF37]/30 shadow-sm mb-4">
              <span className="w-2 h-2 rounded-full bg-[#D4AF37] animate-pulse" />
              <span className="font-display font-bold text-xs tracking-[0.2em] text-[#C5A059] uppercase">
                📸 #ELITEHIJAB &bull; STYLE CHRONICLES
              </span>
            </div>

            <h2 className="font-display font-bold text-3xl sm:text-4xl md:text-5xl text-ink leading-[1.28] py-2 overflow-visible">
              Styled by our{" "}
              <span className="italic font-normal gradient-gold-text inline-block pr-3.5 pb-2 pt-1 -my-1">
                global community
              </span>
            </h2>
          </div>
          ...
          <div className="flex items-center justify-center lg:justify-end gap-3 shrink-0">
            <a
              ...
              className="inline-flex items-center justify-center gap-2.5 px-7 py-3.5 rounded-full bg-[#0A0A0A] text-white border border-[#D4AF37]/50 font-body font-semibold text-sm tracking-wide shadow-card hover:bg-[#D4AF37] hover:text-[#0A0A0A] transition-all duration-300 hover:scale-[1.03]"
            >
        ...
        <div className="mt-8 sm:mt-10 columns-2 md:columns-3 gap-3.5 sm:gap-6 space-y-3.5 sm:space-y-6">
          {lookbook.map((src, i) => (
            ...
            >
              <div
                className="group lift relative w-full rounded-[18px] sm:rounded-[28px] overflow-hidden shadow-card border-[3px] sm:border-[3.5px] border-white hover:border-[#D4AF37]/40 bg-cream-deep cursor-pointer transition-all duration-500 hover:shadow-2xl"
              >
                <Image
                  src={src}
                  alt={`Elite Hijab community styling look ${i + 1}`}
                  width={750}
                  height={1050}
                ...
                <div className="absolute bottom-3 sm:bottom-3.5 left-3 right-3 sm:left-3.5 sm:right-3.5 flex items-center justify-between gap-1.5 z-10">
                  <div className="bg-white/95 backdrop-blur-md px-2.5 sm:px-3.5 py-1.5 sm:py-2 rounded-xl text-ink shadow-md flex items-center justify-between gap-1.5 sm:gap-2 flex-1 min-w-0 border border-[#D4AF37]/20">
                    <div className="flex items-center gap-1.5 sm:gap-2 overflow-hidden min-w-0">
                      <div className="w-4.5 h-4.5 sm:w-5 sm:h-5 rounded-full bg-[#0A0A0A] text-[#D4AF37] flex items-center justify-center font-bold text-[9px] sm:text-[10px] shrink-0 border border-[#D4AF37]/30">
                        ✨
                      </div>
                    ...
                    <span className="text-[10px] sm:text-[11px] text-[#C5A059] font-bold uppercase tracking-wider shrink-0 hidden sm:inline">
                      Verified
                    </span>
                  </div>

                  {/* Mobile-Only Permanent Shop Button */}
                  <a
                    href={`https://wa.me/${SITE.whatsapp}?text=${encodeURIComponent(
                      `Hi Elite Hijab! I loved this community look (${tags[i % tags.length]}) and would like to shop this exact style.`
                    )}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="sm:hidden bg-[#0A0A0A] text-white border border-[#D4AF37]/40 px-3 py-1.5 rounded-xl font-display font-bold text-[10px] uppercase tracking-wider shadow-md active:scale-95 shrink-0 hover:bg-[#D4AF37] hover:text-[#0A0A0A] transition-colors"
                  >
                    Shop Look
                  </a>
                </div>
        ...
        <Reveal className="mt-12 sm:mt-16 text-center">
          <div className="inline-flex flex-col sm:flex-row items-center justify-center gap-4 p-6 sm:p-8 rounded-3xl bg-white/80 backdrop-blur-md border border-gold/30 shadow-soft w-full max-w-2xl mx-auto">
            <div className="text-center sm:text-left">
              ...
              <p className="text-xs sm:text-sm text-ink/70 mt-1">Upload your photo on Instagram with #EliteHijabLook &amp; win monthly vouchers!</p>
            </div>
            <a
