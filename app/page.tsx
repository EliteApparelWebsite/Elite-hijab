...
export default async function Home() {
  ...
  const formattedProducts = (products || []).map((p: any) => ({
    ...
    image_url: p.featured_image_url || p.product_images?.[0]?.image_url || p.image_url || p.image || "/image.png",
  ...
  const formattedSalwarKameez = (salwarKameezProducts || []).map((p: any) => ({
    ...
    slug: p.slug,
    name: p.name,
    image_url: p.featured_image_url || p.product_images?.[0]?.image_url || p.image_url || p.image || "/luxe-salwar-kameez.png",
    price: Number(p.product_variants?.[0]?.price || p.price || 0) || 0,
    oldPrice: p.product_variants?.[0]?.original_price || p.oldPrice || undefined,
    badge: p.badge,
    rating: p.rating || 5,
