...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (!result.ok) {
      ...
      return NextResponse.redirect(`${siteUrl}/orders/track`, { status: 303 })
    }

    return NextResponse.redirect(`${siteUrl}/orders/track?orderNumber=${encodeURIComponent(result.orderNumber!)}`, { status: 303 })
  } catch (error) {
